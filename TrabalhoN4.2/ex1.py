...
class LinkedBinaryTree:
    ...
    def _validate(self, p):
        if not isinstance(p, self.Position): raise TypeError
        if p._container is not self: raise ValueError
        if p._node._parent is p._node: raise ValueError
        return p._node
...
if __name__ == "__main__":
    print("=== EXERCÍCIO 1: TreeMap (Árvore Binária de Busca) ===")
    T = TreeMap()
    print("\n1. Inserindo elementos: 50, 30, 70")
    T[50] = "Raiz"
    T[30] = "Esq"
    T[70] = "Dir"
    
    print("   Tamanho da árvore:", len(T))
    print("   Chaves:", list(iter(T)))